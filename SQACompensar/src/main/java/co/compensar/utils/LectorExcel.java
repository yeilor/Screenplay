...
// Método modificado para manejar fechas
    public static LinkedHashMap<String, String> getCellValue(Sheet sheet, Row row, int currentColumn) {
        ...
        if (row == null) {
            ...
            if (cell.getCellType() == CellType.STRING) {
                ...
                if (DateUtil.isCellDateFormatted(cell)) { // Verifica si la celda es una fecha
                    String columnHeaderName = sheet.getRow(sheet.getFirstRowNum()).getCell(cell.getColumnIndex())
                            .getStringCellValue();
                    // Formato de fecha que desees (puedes modificar el formato)
                    SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");
                    Date dateValue = cell.getDateCellValue();
                    columnMapdata.put(columnHeaderName, dateFormat.format(dateValue));
                } else {
                    String columnHeaderName = sheet.getRow(sheet.getFirstRowNum()).getCell(cell.getColumnIndex())
                            .getStringCellValue();
                ...
                if (cellValue == null || cellValue.getCellType() == CellType.BLANK) {
                    ...
                    columnMapdata.put(columnHeaderName, Double.toString(numericValue));
